module CO4.Test.LocalTermination.PPrint
where
...
import           CO4.Test.LocalTermination.Util (SymbolMap)
...
pprintAutomaton :: (SymbolMap -> s -> String) -> SymbolMap -> Automaton s State 
                -> String
pprintAutomaton goS symbolMap automaton =
  unlines [ unwords $ "states:" : map pprintState (states automaton)
          , "transition: " ++ (pprintModel goS symbolMap model) 
          ]
  where
    model = modelFromAutomaton automaton

pprintState :: State -> String
pprintState = show . value
