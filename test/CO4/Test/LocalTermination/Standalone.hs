module CO4.Test.LocalTermination.Standalone
where

import Prelude hiding (lex,lookup,length,product)
...
constraint :: (DPTrs (), DPTrs (), [Domain], Automaton Symbol State, State) 
           -> Proof
           -> Bool
constraint (originalTrs,trs,modelValues,language,garbage) (Proof model orders) = 
  let isModelOriginal = snd ( makeLabeledTrs model originalTrs modelValues )
  in
    case makeLabeledTrs model originalTrs modelValues of
      (labeledTrs, isModel) -> 
        let nonGarbage = filterGarbage model garbage labeledTrs
        in
          and [ isModelOriginal
          --    , isModel
              , couldDeleteOneRule ( steps ( tagAll nonGarbage ) orders )
              , isSubLanguage (language,garbage) (automatonFromModel modelValues model)
              ]
...
couldDeleteOneRule trs = case trs of
    TaggedGroupedTrs rss -> exists rss ( \ rs -> 
        forall rs ( 
          \(tag,rule) -> not tag && isMarkedRule rule 
        ) )
...
productTransitions :: (sym -> sym -> Bool) -> Transition sym s -> Transition sym s 
                   -> Transition sym (s,s)
productTransitions eqSym a b =
  let goTransition (sym, mA) =
        let mB = lookup eqSym sym b
        in
          (sym, goMapping mA mB)
...
modelFromAutomaton (_,transition) = 
  let goTransition (s, m) = (s, map goMap m)
      goMap (args, image) = (map goArg args, image)
      goArg arg           = Exactly arg
  in
    map goTransition transition

isLhsInLanguage :: Model Symbol -> State -> DPRule Label -> Bool
isLhsInLanguage model garbage rule = case rule of
  Rule _ lhs _ -> case lhs of
    Var _      -> undefined
    Node n l _ -> not (eqValue (valueOfFun n l model) garbage)
...
eqPair :: (s -> s -> Bool) -> (s,s) -> (s,s) -> Bool
eqPair f (a,b) (c,d) = (f a c) && (f b d)
