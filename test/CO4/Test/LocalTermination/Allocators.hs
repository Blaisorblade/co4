...
module CO4.Test.LocalTermination.Allocators
  (allocator)
where
...
import           CO4.Test.LocalTermination.Util (nodeArities)
import           CO4.Test.LocalTermination.Config

$( compileFile [OnlyAllocators, ImportPrelude] "test/CO4/Test/LocalTermination/Standalone.hs" )
...
{-
automatonAllocator :: Config -> Automaton Symbol State -> TAllocator (Automaton Symbol State)
automatonAllocator config automaton = knownTuple2 statesAlloc transitionAlloc
  where
    n               = automatonStateBitWidth config
    statesAlloc     = kList ((2^n) - 1) $ uState n
    transitionAlloc = allocatorList $ map (\(s,m) -> knownTuple2 (kState s) $ mapAlloc m)
                                    $ transition automaton
    mapAlloc        = allocatorList 
                    . map 
                    ( \(args,_) -> knownTuple2 (fromKnown args) $ uState n )
-}
...
kState :: State -> TAllocator State
kState = fromKnown

uState :: Int -> TAllocator State
uState = uNat
